...
use crate::{LooseParser, PaymentParser, RequisiteToleranceParser, StrictParser};
...
pub(super) const FORMAT_ID_BYTES: [u8; 2] = [b'S', b'T'];
pub(super) const VERSION_0001_BYTES: [u8; 4] = [b'0', b'0', b'0', b'1'];

/// Информация о платеже.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct Payment<T: CustomRequisites = NoCustomRequisites> {
    pub(super) header: PaymentHeader,
    pub(super) requisites: Vec<Requisite<T>>,
}
...
impl Payment {
    ...
    /// Парсер, который пропускает неправильные реквизиты.
    pub fn requisite_tolerance_parser() -> PaymentParser<RequisiteToleranceParser> {
        PaymentParser::default()
    }

    /// Нестрогий парсер, пропускает все ошибки.
...
impl<T: CustomRequisites> Payment<T> {
    ...
    /// Строгий парсер с пользовательскими реквизитами.
    pub fn custom_parser() -> PaymentParser<StrictParser, T> {
        PaymentParser::default()
    }

    /// Парсер с пользовательскими реквизитами, который пропускает неправильные реквизиты.
    pub fn requisite_tolerance_custom_parser() -> PaymentParser<RequisiteToleranceParser, T> {
        PaymentParser::default()
    }

    /// Нестрогий парсер с пользовательскими реквизитами, пропускает все ошибки.
    pub fn loose_custom_parser() -> PaymentParser<LooseParser, T> {
        PaymentParser::default()
    }
...
pub struct PaymentHeader {
    ...
    pub(super) format_id: [u8; 2],

    /// Версия стандарта
    pub(super) version: [u8; 4],

    /// Признак набора кодированных знаков
    pub(super) encoding: PaymentEncoding,
